# Task FE-04: Chart Type Recommender

## Metadata
- **ID**: FE-04-chart-type-recommender
- **Layer**: 1
- **Dependencies**: `01-project-scaffolding`
- **Blocks**: `UI-06`, `TEST-02`
- **Parallel With**: BE-01, BE-02, BE-03, FE-01, FE-05, FE-06, UI-12, DOC-*
...
| File | Path |
|------|------|
| `ChartTypeRecommender.cs` | `src/FlexibleMonitoringDashboard.Client/Services/ChartTypeRecommender.cs` |

---

## Step-by-Step Implementation

### Step 1: Create `ChartTypeRecommender.cs`

This client-side service uses heuristics to suggest the best chart type based on detected field types from the backend's analysis response.
...
```
Server /api/proxy/analyze
    │
    ▼
AnalyzeResponse { SuggestedChartType, SuggestedXField, SuggestedYFields, Fields }
    │
    ▼
ChartTypeRecommender.RecommendFromServerSuggestion(suggestedChartType) → ChartType enum
    │
    ▼
ChartTypeRecommender.BuildDefaultMappings(xField, yFields, fields) → List<FieldMapping>
    │
    ▼
User can override in UI → Final WidgetConfig
```
...
- [ ] `RecommendFromServerSuggestion("Line")` → `ChartType.Line`
- [ ] `RecommendFromServerSuggestion("RadialBar")` → `ChartType.RadialBar`
- [ ] `RecommendFromServerSuggestion(null)` → `ChartType.Bar` (fallback)
- [ ] Fields with time array + number array → `ChartType.Line`
- [ ] Single numeric scalar → `ChartType.RadialBar`
- [ ] `BuildDefaultMappings` creates correct XAxis/YAxis role mappings
- [ ] `GetAvailableChartTypes()` returns 7 options with icons
